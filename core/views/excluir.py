...
###################################################################################################
def delete_ordem_compra(request, id):
    delete_ordem_compra = Compra.objects.get(id=id)

    if request.method == 'POST':
        delete_ordem_compra.delete()
        return redirect('list_ordem_compra')
    
    return render(request, "exclusaoConf.html", {'delete_ordem_compra': delete_ordem_compra})

def delete_gerenciar_compra(request, id):
    delete_gerenciar_compra = Compra.objects.get(id=id)

    if request.method == 'POST':
        delete_gerenciar_compra.delete()
        return redirect('list_gerenciar_compra')
    
    return render(request, "exclusaoConf.html", {'delete_gerenciar_compra': delete_gerenciar_compra})
