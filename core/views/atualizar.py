...
##################################################################################################################
def update_ordem_compra(request, id):
    update_ordem_compra = Compra.objects.get(id=id)
    form = CompraForm(request.POST or None, instance=update_ordem_compra)

    if form.is_valid():
        form.save()
        return redirect('list_ordem_compra')

    return render(request, 'grupo/ordemcompra.html', { 'form' : form, 'update_ordem_compra':update_ordem_compra })


def update_gerenciar_compra(request, id):
    update_gerenciar_compra = Compra.objects.get(id=id)
    form = CompraForm(request.POST or None, instance=update_gerenciar_compra)

    if form.is_valid():
        form.save()
        return redirect('list_gerenciar_compra')

    return render(request, 'grupo/gerenciarcompra.html', { 'form' : form, 'update_gerenciar_compra':update_gerenciar_compra })
